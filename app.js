...
async function submitReq(e){
  ...
  try{
    ...
    if(fd.get('prioridade')==='critico'&&!String(fd.get('observacoes')||'').trim()){toast('Prioridade crítica exige observação.'); return;}
    const {data:num,error:numErr}=await sb.rpc('proximo_numero_requisicao');
    if(numErr) throw numErr;
    const req={numero:num,setor,responsavel_nome:String(fd.get('responsavel')||userName()).trim(),data_requisicao:fd.get('data'),prioridade:fd.get('prioridade'),observacoes:String(fd.get('observacoes')||'').trim()||null,criado_por:state.session.user.id,criado_por_nome:userName()};
    const {data:r,error}=await sb.from('requisicoes').insert(req).select('*').single();
    if(error) throw error;
    const rows=selected.map(i=>({requisicao_id:r.id,catalog_item_id:i.id,categoria:i.categoria_nome,nome:i.nome,unidade:i.unidade,qtd_solicitada:Number(fd.get('q_'+i.id)),observacao_item:String(fd.get('obs_'+i.id)||'').trim()||null}));
    ...
    if(btn){btn.disabled=false;btn.textContent=oldText||'Enviar Requisição Online';}
  }
}
